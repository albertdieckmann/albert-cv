...
export async function POST(req: NextRequest) {
  ...
  const { groupId, showId, showTitle, status, performanceStart, performanceEnd } =
    await req.json();
  ...
  if (!status) {
    ...
    await sql`
      INSERT INTO fringe_picks
        (group_id, user_id, user_name, show_id, show_title, status, performance_start, performance_end, updated_at)
      VALUES
        (${groupId}, ${userId}, ${userName}, ${showId}, ${showTitle ?? ""}, ${status},
         ${performanceStart ?? null}, ${performanceEnd ?? null}, NOW())
