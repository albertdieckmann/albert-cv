...
import { loadUi, saveUi, api, fmtEdinburgh, PAGE_SIZE, toComparableIso, computeConflicts } from "./utils";
import { visibleShows, planGroups, allGenres, allAreas, allDays, perfsInRange, perfPicksFor } from "./filters";
...
export default function FringePage() {
  ...
  function handleStatusClick(show: Show, status: PickStatus) {
    ...
    const rangePerfs = perfsInRange(show, dateFrom, dateTo);
  ...
  const allVisible  = visibleShows(shows, session, { search, selectedOnly, hideInterested, genreFilter, areaFilter, dateFrom, dateTo });
  const visible     = allVisible.slice(0, visibleCount);
  const hasMore     = visibleCount < allVisible.length;
  const pGroups     = planGroups(shows, session, { hideInterested, dateFrom, dateTo });
  const conflicts   = computeConflicts(session.activeGroup?.picks ?? []);
  const tCount      = pGroups.flatMap(([, items]) => items).length;
  const genres      = allGenres(shows);
  const areas       = allAreas(shows);
  const days        = allDays(shows, session);
  ...
  return (
    <div className={`${s.page} ${busy ? s.busy : ""}`}>
      ...
      <PlanTab
        ...
        perfsInRange={(show) => perfsInRange(show, dateFrom, dateTo)}
        perfPicksFor={(showId, perfStart) => perfPicksFor(session, showId, perfStart)}
